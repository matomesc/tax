...
merge(Task.prototype, buildProto(), {
  ...
  watch: function (include, exclude) {
    ...
    wrapper = function wrapper(done) {
      var watching = [];
      Seq(include)
        .parEach(function (_glob) {
          var self = this;
          glob(_glob, function (err, files) {
            if (err) {
              return self(err);
            }
            including = [].concat.call(including, files);
            return self();
          });
        })
        .seq(function () {
        ...
        .parEach(function (_glob) {
          ...
          glob(_glob, function (err, files) {
            ...
            excluding = [].concat.call(excluding, files);
            return self();
          });
        })
        .seq(function () {
        ...
        .empty()
        .seq(function (inc) {
          including.forEach(function (inc) {
            if (excluding.indexOf(inc) === -1) {
              watching.push(inc);
            }
          });
          this();

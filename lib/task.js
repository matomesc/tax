...
_.extend(Task.prototype, buildProto(), {
  ...
  //
  run: function (args) {
    // prevent us from running while we're running
    if (this._running) {
      return;
    }
    this._running = true;
    ...
    Seq(self.queue)
      .seqEach(function (subtask) {
        var next = this;
        // don't attach listeners twice
        if (self._attachedListeners != self.queue.length) {
          self._attachedListeners += 1;
  ...
  exec: function (cmd, execOpts) {
    function wrapper(cb) {
      ...
      execOpts = execOpts && _.extend(defaultOpts, execOpts) || defaultOpts;
